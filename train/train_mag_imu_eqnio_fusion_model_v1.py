    ...
    *,
    ...
    for batch in loader or []:
        ...
        with torch.cuda.amp.autocast(enabled=(use_amp and device.type == "cuda")):
            # 两次 forward：原始 + yaw增强（由 transform 生成 batch["aug"]）
...
def main():
    ...
    # # --------- transforms：numpy->tensor + yaw augment ---------
    # def feature_transform(sample: Dict[str, Any]):
    #     sample = DefaultTransform()(sample)
    #     # 建议先只用 SO(2)（p_reflect=0），跑通后再开反射增强
    #     sample = YawAugmentO2Transform(p_reflect=0.0)(sample)
    #     return sample
    # 建议先只用 SO(2)（p_reflect=0），跑通后再开反射增强
